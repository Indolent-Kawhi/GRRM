...
from utils.collator import TestCollator
from utils.prompt import all_prompt
...
def test_ddp(args):
    ...
    # Ensure the model is moved to the correct GPU device
    ...
    test_loader = DataLoader(test_data, batch_size=args.test_batch_size, collate_fn=collator,
                             sampler=ddp_sampler, num_workers=4, pin_memory=True)
    ...
    with torch.no_grad():
        ...
        for prompt_id in prompt_ids:
            ...
            for step, batch in enumerate(tqdm(test_loader)):
                ...
                predictions = [_.split("\n")[-1].split("<|im_end|>")[-2] for _ in output]

                topk_res = get_topk_results(predictions, scores, targets, num_beams,
                ...
                if local_rank == 0:
                    ...
                    for m, res in batch_metrics_res.items():
                        if m not in metrics_results:
                            metrics_results[m] = res * sum(bs_gather_list)
                        else:
                            metrics_results[m] += res * sum(bs_gather_list)
