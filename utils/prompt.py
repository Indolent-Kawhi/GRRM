...
rl_prompt = "User Purchase History: {history}.\nPlease now take the User Purchase History and make a recommendation."
...
# ========================================================
# Task 7 -- ItemFeatureObtain -- Prompt
# ========================================================

itemfeatureobtain_prompt = []

#####——0
prompt = {}
prompt["instruction"] = "As a recommendation system, generate a set of descriptive tags for the following item: {item}."
prompt["response"] = "{features}"
itemfeatureobtain_prompt.append(prompt)

#####——1
prompt = {}
prompt["instruction"] = "To improve product matching, analyze {item} and output its key attributes as a comma-separated list."
prompt["response"] = "{features}"
itemfeatureobtain_prompt.append(prompt)

#####——2
prompt = {}
prompt["instruction"] = "Populate the feature profile for {item}. These features will be used to find similar products."
prompt["response"] = "{features}"
itemfeatureobtain_prompt.append(prompt)

#####——3
prompt = {}
prompt["instruction"] = "As a recommendation engine, extract the most important user-identified characteristics for item {item}."
prompt["response"] = "{features}"
itemfeatureobtain_prompt.append(prompt)

#####——4
prompt = {}
prompt["instruction"] = "Analyze and list the defining features for {item} so it can be correctly categorized and recommended."
prompt["response"] = "{features}"
itemfeatureobtain_prompt.append(prompt)

#####——5
prompt = {}
prompt["instruction"] = "A user wants a product with the following characteristics. Recommend the best match. Characteristics: {features}."
prompt["response"] = "{item}"
itemfeatureobtain_prompt.append(prompt)

#####——6
prompt = {}
prompt["instruction"] = "Based on the feature set below, identify the corresponding product for a user recommendation. Features: {features}."
prompt["response"] = "{item}"
itemfeatureobtain_prompt.append(prompt)

#####——7
prompt = {}
prompt["instruction"] = "Execute a product search. The query parameters are the following attributes. Return the single best product match. Query: {features}."
prompt["response"] = "{item}"
itemfeatureobtain_prompt.append(prompt)

#####——8
prompt = {}
prompt["instruction"] = "A user has selected these tags to find a product. Your job is to identify and recommend the item. Tags: {features}."
prompt["response"] = "{item}"
itemfeatureobtain_prompt.append(prompt)

#####——9
prompt = {}
prompt["instruction"] = "Given this list of desired features from a user query, find the correct item in the catalog. Features: {features}."
prompt["response"] = "{item}"
itemfeatureobtain_prompt.append(prompt)

all_prompt["itemfeatureobtain"] = itemfeatureobtain_prompt

# ========================================================
# Task 8 -- HistoryReconstruction -- Prompt
# ========================================================

history_reconstruction_prompt = []

# Category 1: Given inters, repeat inters
#####——0
prompt = {}
prompt["instruction"] = "Please repeat the following user interaction history: {inters}."
prompt["response"] = "{inters}"
history_reconstruction_prompt.append(prompt)

#####——1
prompt = {}
prompt["instruction"] = "Given the user's historical interactions: {inters}. Please restate this history."
prompt["response"] = "{inters}"
history_reconstruction_prompt.append(prompt)

#####——2
prompt = {}
prompt["instruction"] = "Paraphrase the following sequence of item interactions for the user: {inters}."
prompt["response"] = "{inters}"
history_reconstruction_prompt.append(prompt)

#####——3
prompt = {}
prompt["instruction"] = "Can you output the user's interaction history again? The history is: {inters}."
prompt["response"] = "{inters}"
history_reconstruction_prompt.append(prompt)

#####——4
prompt = {}
prompt["instruction"] = "Your task is to accurately reproduce the user's interaction history provided here: {inters}."
prompt["response"] = "{inters}"
history_reconstruction_prompt.append(prompt)


# Category 2: Given inter_titles, output inters
#####——5
prompt = {}
prompt["instruction"] = "The user has interacted with items titled: {inter_titles}. What are the corresponding items for this history?"
prompt["response"] = "{inters}"
history_reconstruction_prompt.append(prompt)

#####——6
prompt = {}
prompt["instruction"] = "Convert the following sequence of item titles into their respective items: {inter_titles}."
prompt["response"] = "{inters}"
history_reconstruction_prompt.append(prompt)

#####——7
prompt = {}
prompt["instruction"] = "Given the user's interaction history by item titles: {inter_titles}. Please provide the sequence of items."
prompt["response"] = "{inters}"
history_reconstruction_prompt.append(prompt)

#####——8
prompt = {}
prompt["instruction"] = "I have a list of item titles from a user's history: {inter_titles}. Can you map them to their item?"
prompt["response"] = "{inters}"
history_reconstruction_prompt.append(prompt)

#####——9
prompt = {}
prompt["instruction"] = "For the interaction history represented by these titles: {inter_titles}, what is the corresponding history in terms of item?"
prompt["response"] = "{inters}"
history_reconstruction_prompt.append(prompt)


# Category 3: Given inters, output inter_titles
#####——10
prompt = {}
prompt["instruction"] = "A user has interacted with the following items: {inters}. What are the titles of these items in order?"
prompt["response"] = "{inter_titles}"
history_reconstruction_prompt.append(prompt)

#####——11
prompt = {}
prompt["instruction"] = "Given the item ID sequence of a user's history: {inters}, please provide the corresponding sequence of item titles."
prompt["response"] = "{inter_titles}"
history_reconstruction_prompt.append(prompt)

#####——12
prompt = {}
prompt["instruction"] = "Can you list the titles for the items in this interaction history: {inters}?"
prompt["response"] = "{inter_titles}"
history_reconstruction_prompt.append(prompt)

#####——13
prompt = {}
prompt["instruction"] = "Map the following item ID history to their titles: {inters}."
prompt["response"] = "{inter_titles}"
history_reconstruction_prompt.append(prompt)

#####——14
prompt = {}
prompt["instruction"] = "What are the titles for the items in this user's interaction record: {inters}?"
prompt["response"] = "{inter_titles}"
history_reconstruction_prompt.append(prompt)


all_prompt["historyreconstruction"] = history_reconstruction_prompt