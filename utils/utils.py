...
def parse_dataset_args(parser):
    parser.add_argument("--data_path", type=str, default="./data",
                        help="data directory")
    parser.add_argument("--tasks", type=str, default="seqrec,item2index,index2item,fusionseqrec,itemsearch,preferenceobtain,itemfeatureobtain",
    ...
    parser.add_argument("--train_prompt_sample_num", type=str, default="1,1,1,1,1,1,1",
                        help="the number of sampling prompts for each task")
    parser.add_argument("--train_data_sample_num", type=str, default="0,0,0,100000,0,0,0",
                        help="the number of sampling prompts for each task")
...
def parse_test_args(parser):
    ...
    parser.add_argument("--ckpt_path", type=str,
                        default="/home/zhouzetong/Rec-RL/LLaMA-Factory/saves/qwen3-4b-instruct/sft/instruments_cold",
    ...
    parser.add_argument("--metrics", type=str, default="recall@1,recall@5,recall@10,ndcg@5,ndcg@10,precision@5,precision@10",
                        help="test metrics, separate by comma")
    parser.add_argument("--test_task", type=str, default="SeqRec-rl")
...
def load_datasets(args):
    ...
    for task, prompt_sample_num,data_sample_num in zip(tasks,train_prompt_sample_num,train_data_sample_num):
        if task.lower() == "seqrec":
            ...
            print("Loaded FusionSeqRec dataset")
            print(f"Length of dataset: {len(dataset)}")
            print(f"Example data: {dataset[0]}")
            ...
            print("Loaded ItemSearch dataset")
            print(f"Length of dataset: {len(dataset)}")
            print(f"Example data: {dataset[0]}")
            ...
            print("Loaded PreferenceObtain dataset")
            print(f"Length of dataset: {len(dataset)}")
            print(f"Example data: {dataset[0]}")

        elif task.lower() == "itemfeatureobtain":
            dataset = ItemFeatureObtainDataset(args, prompt_sample_num=prompt_sample_num, sample_num=data_sample_num)
            print("Loaded ItemFeatureObtain dataset")
            print(f"Length of dataset: {len(dataset)}")
            print(f"Example data: {dataset[0]}")
            
        elif task.lower() == "historyreconstruction":
            dataset = HistoryReconstructionDataset(args, prompt_sample_num=prompt_sample_num, sample_num=data_sample_num)
            print("Loaded HistoryReconstruction dataset")
            print(f"Length of dataset: {len(dataset)}")
            print(f"Example data: {dataset[0]}")
...
def load_test_dataset(args):
    ...
    elif args.test_task.lower() == "seqrec-rl":
        test_data = SeqRecRLDataset(args, mode="test", sample_num=args.sample_num)
    else:
        raise NotImplementedError
