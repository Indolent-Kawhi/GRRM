...
import pdb
import json
from utils.prompt import sft_prompt, all_prompt
import numpy as np
...
class BaseDataset(Dataset):
    ...
    def get_prefix_allowed_tokens_fn(self, tokenizer):
        ...
        if self.allowed_tokens is None:
            ...
            self.allowed_tokens[len(self.allowed_tokens.keys())-1].add(tokenizer.eos_token_id)
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            while True:
                prompt_id = random.randint(0, len(self.prompts) - 1)
                prompt = self.prompts[prompt_id]
                if ("{description}" in prompt["instruction"] or "{description}" in prompt["response"]) and (d["description"] == "" or d["description"] == "."):
                    continue
                else:
                    break
        elif self.mode == 'test':
...
class SeqRecRLDataset(BaseDataset):
    ...
    def get_prefix_allowed_tokens_fn(self, tokenizer):
        ...
        sep = tokenizer("answer>")["input_ids"]
        eos = tokenizer.eos_token_id
    ...
    def __getitem__(self, index):
        ...
        # input = rl_prompt.format(history=d["inters"])
        prompt_id = self.prompt_id
        prompt = self.prompts[prompt_id]
        input = prompt["instruction"].format(**d)
        output = d["item"]

        # print({"input": input, "output": output})

        return dict(input_ids=input, labels=output, history=d["inters"])
