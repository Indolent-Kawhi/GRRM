...
from scipy.special import comb # pass@k training
...
# pass@k training
def calc_adv(val, k):
    c = len(np.where(val==1.0)[0])
    n = len(val)
    rho = 1 - comb(n-c, k) / comb(n, k)
    sigma = np.sqrt(rho * (1 - rho))
    adv_p = (1 - rho) / (sigma + 1e-6)
    adv_n = (1 - rho - comb(n-c-1, k-1)/comb(n-1,k-1)) / (sigma + 1e-6)
    new_val = np.where(val==1.0, adv_p, adv_n)
    # new_val = np.where(val==1.25, adv_p, val)
    # new_val = np.where(new_val==0, adv_n, new_val)
    return new_val

@register_adv_est(AdvantageEstimator.PASSK)  # or simply: @register_adv_est("passk")
def compute_passk_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
